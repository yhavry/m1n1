...
class IOReporting_Start(IOReportingMessage):
    TYPE = 55, 52, Constant(0xc)

class IOReporting_Report(IOReportingMessage):
    TYPE = 55, 52, Constant(0x8)

class AKFIOReportingEndpoint(AKFBaseEndpoint):
    ...
    def GetBuf(self, msg):
        ...
        if msg.DVA != 0:
            ...
            self.iobuffer, self.iobuffer_dva = self.akf.ioalloc(self.bufsize)
    ...
    def Init(self, msg):
        self.log("report!")
        buf = self.akf.iface.readmem(self.iobuffer, self.bufsize)
        #chexdump(buf)
